...
def _load_utils_module(mocker):
    module_path = Path(__file__).resolve().parents[2] / "src" / "streaming" / "utils.py"
    return _load_module("streaming_utils_test_module", module_path, mocker)


def _load_curation_module(mocker):
    module_path = (
        Path(__file__).resolve().parents[2] / "src" / "streaming" / "curation.py"
    )
    return _load_module("streaming_curation_test_module", module_path, mocker)
...
def test_consumer_extract_curated_record_keeps_only_required_fields(mocker):
    curation_module = _load_curation_module(mocker)
    payload = {
        "data": {
            "idx": 42,
            "aqi": 64,
    ...
    result = curation_module.extract_curated_record(payload)
...
def test_consumer_write_curated_records_skips_cached_and_batch_duplicates(
    ...
    filtered_records, updated_cache = curation_module.filter_curated_records(
        curated_records,
        curation_module.load_curated_observation_cache(cache_path),
        logger=logger,
    )

    assert filtered_records == [{"timestamp": "2026-04-07T11:00:00+03:00", "station_id": 42}]
...
def test_consumer_write_curated_records_skips_missing_dedup_fields(tmp_path, mocker):
    ...
    assert filtered_records == [{"timestamp": "2026-04-07T10:00:00+03:00", "station_id": 42}]
...
def test_consumer_raises_for_invalid_persisted_cache_file(tmp_path, mocker):
    ...
    with pytest.raises(json.JSONDecodeError):
        curation_module.load_curated_observation_cache(
...
def test_consumer_persists_cache_with_atomic_replace(tmp_path, mocker):
    ...
    curation_module.persist_curated_observation_cache(
        tmp_path / "curated_observation_cache.json",
...
def test_join_hdfs_root_normalizes_slashes(mocker):
    utils_module = _load_utils_module(mocker)

    result = utils_module.join_hdfs_root("data\\air-quality\\", "/sofia/", "raw")

    assert result == "/data/air-quality/sofia/raw"


def test_normalize_hdfs_path_normalizes_hdfs_uri_with_backslashes(mocker):
    utils_module = _load_utils_module(mocker)

    result = utils_module.normalize_hdfs_path("hdfs://namenode:9000\\data\\air-quality\\")

    assert result == "/data/air-quality"


def test_get_geo_value_returns_requested_coordinate(mocker):
    curation_module = _load_curation_module(mocker)

    result = curation_module.get_geo_value({"city": {"geo": [42.6977, 23.3219]}}, 1)

    assert result == 23.3219


def test_extract_day_falls_back_to_processing_date(mocker):
    curation_module = _load_curation_module(mocker)

    result = curation_module.extract_day({"data": {"idx": 7}}, "2026-04-07")
